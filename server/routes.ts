...
async function enforceFreeLlmQuota(
  req: any,
  ...
  const [row] = await db
    .select()
    .from(llmUsage)
    .where(eq(llmUsage.subjectId, subjectId))
    .limit(1);
...
async function searchWikipediaRu(
  query: string,
  limit = 3,
  ...
  try {
    ...
    const res = await fetch(url.toString(), {
      headers: { "User-Agent": "agent_intergator/1.0" },
    });
    if (!res.ok) return [];
    ...
    try {
      ...
      if (!quota.allowed) {
        return res.status(402).json({
          message:
            "Лимит бесплатных запросов исчерпан. Оплатите дополнительные запросы.",
          paywall: true,
...
`;
      ...
      const ranked: Array<{ id: number; score: number; reason: string }> =
        ...
        .map((x) => ({
          ...
          score:
            typeof x.score === "number"
              ...
              : 0.5,
          reason:
            typeof x.reason === "string" ? x.reason : "Подходит под запрос.",
        }));
    ...
    try {
      ...
      if (!llm) {
        return res
          ...
          .json({
            message:
              "LLM is not configured (missing AI_INTEGRATIONS_OPENAI_API_KEY)",
          });
      }
  ...
  app.post(
    ...
    async (req: any, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: "Failed to upsert knowledge base doc" });
      }
    },
  );
  ...
  app.post(api.knowledgeBase.ask.path, async (req, res) => {
    try {
      ...
      const external = includeExternal
        ? await searchWikipediaRu(input.query, 3)
        : [];
...
`;
      ...
      const ids: number[] = Array.isArray(raw.recommendedAgentIds)
        ? raw.recommendedAgentIds.filter((x: any) => typeof x === "number")
        : [];
      const byId = new Map(allAgents.map((a) => [a.id, a] as const));
      const recommendedAgents = ids
        .map((id) => byId.get(id))
        .filter(Boolean)
        .slice(0, 3) as any[];
  ...
  app.post(api.customRequests.analyze.path, async (req, res) => {
    try {
      ...
      const contextData = customReq.contextData
        ? JSON.parse(customReq.contextData)
        : [];
      ...
      try {
        assessment = JSON.parse(
          assessmentResponse.choices[0]?.message?.content || "{}",
        );
      } catch {
      ...
      if (
        !assessment.hasEnoughContext &&
        assessment.questionToAsk &&
        followUpCount < 3
      ) {
        const newContextData = [
          ...contextData,
          { question: assessment.questionToAsk, answer: null },
        ];
      ...
      const updatedReq = await storage.updateCustomRequestStatus(
        customReq.id,
        "completed",
        normalizeToString(result.recommendation) ||
          "Мы рекомендуем AI-агента для автоматизации ваших бизнес-процессов.",
