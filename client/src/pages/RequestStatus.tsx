...
export default function RequestStatus() {
  ...
  useEffect(() => {
    if (request?.status === 'pending' && !analyzeMutation.isPending && !analyzeMutation.isSuccess) {
      ...
      <main className="flex-grow py-16 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto w-full">
        {isLoading ? (
          ...
          <div className="glass-panel rounded-3xl p-8 text-center border-red-100">
            ...
            <p className="text-slate-600">We couldn't find the custom request you're looking for.</p>
          </div>
        ) : (
          <div className="space-y-8">
            ...
            <motion.div 
              ...
              className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm flex flex-col md:flex-row gap-6 justify-between items-center"
            >
              <div>
                ...
                <p className="text-slate-500 text-sm">Request #{request.id.toString().padStart(4, '0')}</p>
              </div>
              ...
              <div className="bg-slate-900 rounded-[22px] p-8 md:p-10 text-white min-h-[400px] relative overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-8 border-b border-white/10 pb-6 relative z-10">
                  <div className="flex items-center gap-3">
                    {activeTab === 'recommendation' ? <TerminalSquare size={24} className="text-accent" /> : <FileCode size={24} className="text-accent" />}
                    <h2 className="text-xl font-bold font-display">
                      {activeTab === 'recommendation' ? 'AI Architect Recommendation' : activeTab === 'code' ? 'Generated Agent Code' : 'Installation Guide'}
                    </h2>
                  </div>
                  {!isAnalyzing && isCompleted && activeTab !== 'recommendation' && (
                    <button 
                      onClick={() => downloadFile(activeTab === 'code' ? 'agent.py' : 'README.md', activeTab === 'code' ? request.generatedCode! : request.generatedReadme!)}
                ...
                <div className="relative z-10">
                  {needsClarification ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20">
                      ...
                      <h3 className="text-xl font-semibold mb-2">Processing your workflow...</h3>
                      <p className="text-slate-400 max-w-md text-center">
                        Our intelligence engine is mapping your business needs to optimal AI agent architectures and generating deployment-ready code.
                      </p>
                    </div>
                  ) : (
                    <motion.div 
            ...
            >
              <h3 className="font-semibold text-slate-900 mb-4 uppercase tracking-wider text-sm">Original Submission</h3>
